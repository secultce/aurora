...
function selectEntity(target) {
    ...
    document.querySelectorAll(`input[name="${fieldName}"]`).forEach(el => el.nextElementSibling.textContent = '');

    const entityInput = document.querySelector(`input[name="${fieldName}"][value^="${entityType}_"]`);
