...
final class Version20241105120541 extends AbstractMigration
{
    ...
    {
        $inscriptionStatusType = <<<EOF
                DO $$ BEGIN
                    CREATE TYPE "enumInscriptionOpportunityStatusType" AS ENUM ('active', 'inactive', 'suspended');
                EXCEPTION
                    WHEN duplicate_object THEN null;
                END $$;
            EOF;

        $this->addSql($inscriptionStatusType);

        $this->addSql('CREATE TABLE inscription_opportunity (id UUID NOT NULL, agent_id UUID NOT NULL, opportunity_id UUID NOT NULL, status "enumInscriptionOpportunityStatusType" NOT NULL, created_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, updated_at TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT NULL, deleted_at TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT NULL, PRIMARY KEY(id))');
        ...
        $this->addSql('COMMENT ON COLUMN inscription_opportunity.status IS \'(DC2Type:inscriptionStatus)\'');
